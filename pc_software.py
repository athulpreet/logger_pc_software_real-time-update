...
class LogDownloaderApp:
    ...
    def send_simple_command(self, command):
        ...
        try:
            with serial.Serial(self.serial_port, BAUD_RATE, timeout=SERIAL_READ_TIMEOUT) as ser:
                ...
                cmd_name = command.decode('utf-8', errors='ignore').strip()
                self.log_text.insert(tk.END, f"\n--- Sent command: {cmd_name} ---\n")
    ...
    def send_time_setting(self):
        ...
        try:
            ...
            current_time = datetime.now().strftime("%Y%m%d%H%M%S")  # Format as YYYYMMDDHHMMSS
    ...
    def process_log_data(self, raw_data):
        ...
        try:
            ...
            # Display summary in the log text
