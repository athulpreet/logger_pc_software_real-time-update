...
class HeaderDetailsWindow:
    ...
    def send_header_data(self):
        ...
        # Format header data according to KS 2295-1:2023 standard
        ...
        try:
            with serial.Serial(self.parent.serial_port, BAUD_RATE, timeout=SERIAL_READ_TIMEOUT) as ser:
                # Send header data directly (no flag needed for header data)
                ser.write(header_string.encode('utf-8'))
                ...
                self.parent.log_text.insert(tk.END, f"Header String:\n{header_string}\n")
...
class LogDownloaderApp:
    ...
    def process_log_data(self, raw_data):
        """Process the raw log data into a structured format"""
        try:
            # Only clear previous log data if this is a complete download
            # For individual commands, we want to append to existing data
            if self.is_downloading:
                self.log_data = []
            ...
            with open(file_path, 'w', newline='') as csvfile:
                ...
                for entry in self.log_data:
                    if entry.get('parsed', False):
                        ...
                        fields.remove('parsed')  # Don't include the parsed flag
                        break
        ...
        try:
            ...
            for row_num, entry in enumerate(self.log_data, 2):
                if entry.get('parsed', False):
                    ...
                    # For unparsed entries, write the raw line
                    raw_line_col = fields.index('raw_line') + 1
    ...
    def export_pdf(self):
        ...
        try:
            ...
            # Add log entries
            c.setFont("Courier", 9)  # Monospaced font for data
